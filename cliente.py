...
IP_SATELITE = os.getenv('DOOM_SAT_ADDR', '192.168.6.229')  # Ip raspberry
...
def reparar_cabecera_hamming(byte_corrupto):
    """
    Decodifica un byte usando Hamming(7,4) entrelazado.
    Calcula el 'Síndrome' para detectar si la radiación (ruido de red)
    ha volteado algún bit y lo repara quirúrgicamente.
    """
    ...
    if sindrome != 0:
        ...
        # Volteamos (XOR 1) el bit de datos correspondiente si está roto
        if sindrome == 3: d1 ^= 1
        elif sindrome == 5: d2 ^= 1
        elif sindrome == 6: d3 ^= 1
        elif sindrome == 7: d4 ^= 1

    # Devolvemos los 4 bits de la cabecera original, limpios
...
while ejecutando:
    ...
    if cambiado:
        frames_recibidos += 1
        unpacked_bytes = b''.join(LUT[b] for b in frame_buffer) # Expandimos 4b a 8b
    ...
    if tiempo_actual - tiempo_calculo >= 1.0:
        ...
        #Render textos
        texto_kbps = FUENTE.render(f"RX  : {kbps:.2f} Kb/s", True, (255,255,255), (0, 0, 0))
        texto_fps  = FUENTE.render(f"FPS : {fps_actuales}", True, (255,255,255), (0, 0, 0))
